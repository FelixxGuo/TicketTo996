 ...
 * integer dist. A person who is "it" at index i can catch any one person whose
 * index is in the range [i - dist, i + dist] (inclusive) and is not "it".
 *
 ...
 * i+dist] = [1-3, 1+3] = [-2, 4]. They can catch the person who is not "it" at
 * index 2. The person who is "it" at index 3 can catch people in the range
 * [i-dist, i+dist] = [3-3, 3+3] = [0, 6]. They can catch the person who is not
 * "it" at index 0. The person who is not "it" at index 4 will not be caught
 * because the people at indices 1 and 3 are already catching one person.
 * Example 2:
 *
 * Input: team = [1], dist = 1
 * Output: 0

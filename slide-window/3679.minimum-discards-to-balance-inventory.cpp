/*
 * @lc app=leetcode id=3679 lang=cpp
 *
 ...
 * If keeping the arrival on day i would cause its type to appear more than m
...
public:
  int minArrivalsToDiscard(vector<int> &arrivals, int w, int m) {
    ...
    for (right = 0; right < n; ++right) {
      ...
      if (!used[left]) {
        if (--uMap[arrivals[left]] == 0) {
          uMap.erase(arrivals[left]);
        }
      }
    }
    return drop;

...
public:
  int maxTotalFruits(vector<vector<int>> &fruits, int startPos, int k) {
    ...
    int left = lower_bound(
        fruits.begin(), fruits.end(), startPos - k,
        [](const vector<int> &a, int value) { return a[0] < value; }) - fruits.begin();
    int right = left, sum = 0, ans = 0;
    ...
    for (; right < n && fruits[right][0] <= startPos + k; ++right) {
      sum += fruits[right][1];
      // left to right: startPost - left + right - left =
      // startPos + fruits[right][0] - fruits[left][0] * 2;
      // right to left: right - startPos + right - left =
      // fruits[right][0] * 2 - startPos - fruits[left][0];
      while (startPos + fruits[right][0] - fruits[left][0] * 2 > k &&
